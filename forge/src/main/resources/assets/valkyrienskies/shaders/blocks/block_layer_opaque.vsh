...
// normal into world space. Needed when shade is on, when world-light runs,
// or when the per-vertex ship-on-ship seam-AO pass needs to lift this
// quad's local corners into world space.
...
// world space. Needed by FSH light pipeline (via v_CameraRelWorldPos), by
// the biome lookup (worldPosVertex), or (with VS_SHIP_ON_SHIP) to get this
// vertex's absolute world position for the seam-AO candidate scan.
