#!/usr/bin/env python3
"""Line-for-line transliteration of the planned GLSL loop for seam4
(occluder-lattice AO with cross-ship merging): single gather pass with
fixed-size arrays, per-run data, on-the-fly claims/r, float occupancy window,
symmetric quadrant corner counts. Fuzzed against seam4.ship_ao."""
import numpy as np
...
def glsl_ship_ao(frag, normal, occluders, self_ship=-1):
    ...
    for si in range(runCount):
        ...
        occ = np.zeros(18)                      # [band*9 + (dv+1)*3 + (du+1)]
        for vi in range(voxCount):
            owner = voxRun[vi]
            if runShip[owner] == self_ship:
                continue
            if owner == si:
                wgt = rv[vi] * fade[vi]
            else:
        ...
        for ship in range(rng.integers(1, 4)):
            ...
            for c in cells:
                occs.append((origin + quat_rotate(q, np.array(c, float)),
                             q, ship + 1))
        self_ship = int(rng.integers(0, 3))  # sometimes matches a ship, often not
